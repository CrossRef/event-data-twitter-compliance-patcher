...
(defn first-compliance-log-chunk
  ...
  []
  (when-let [batch-key (last (store/keys-matching-prefix @storage "twitter/tweet-deletions/"))]
    (log/info "Looking at batch" batch-key)
    (let [ids (json/read-str (store/get-string @storage batch-key))
          num-ids (count ids)
